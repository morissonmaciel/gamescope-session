...
if ! git clone "$repo_url" "$HOME/.gamescope-setup"; then
    ...
    if ! wget -O "$HOME/.gamescope-setup/archive.zip" "https://github.com/morissonmaciel/gamescope-session/archive/refs/tags/v0.10.zip"; then
...
# Execute setup.sh script as root
sudo bash setup.sh
